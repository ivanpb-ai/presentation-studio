...
// Gated like deck storage — a Netlify Identity bearer token or the sign-in
// cookie — so anonymous visitors cannot spend the site's Replicate credits;
// when the site is fully open (no Identity, no cookie users) the proxy is
// open too, matching the editor itself.
//
...
import { parseUsers, cookieAuthedUser, identityAuthedUser } from "./decks.mjs";
...
// Is Netlify Identity enabled on this site? Cached across warm invocations.
let idEnabled = null;
async function identityEnabled(req) {
  if (idEnabled !== null) return idEnabled;
  const base = process.env.URL || new URL(req.url).origin;
  try { idEnabled = (await fetch(base + "/.netlify/identity/settings")).ok; }
  catch { idEnabled = false; }
  return idEnabled;
}

async function isAuthed(req) {
  if (await identityAuthedUser(req)) return true;
  if (cookieAuthedUser(req)) return true;
  // Fully open site: no cookie accounts and Identity not enabled.
  return !parseUsers().size && !(await identityEnabled(req));
}
