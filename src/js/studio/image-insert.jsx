...
async function searchOpenverse(q, page) {
  ...
  let viaErr;
  try {
    const r = await fetch(`${API}?ov_q=${encodeURIComponent(q)}&page=${page}`, { credentials: "same-origin", headers: { ...(await authHeader()) } });
...
async function generateImage(prompt, aspect) {
  ...
  while (r.ok && d && !d.dataUri && !d.error && d.id && Date.now() - t0 < 120000) {
    await sleep(2500);
    r = await fetch(`${API}?id=${encodeURIComponent(d.id)}`, { credentials: "same-origin", headers: { ...(await authHeader()) } });
