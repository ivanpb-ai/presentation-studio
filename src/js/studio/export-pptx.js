...
// HTML→PowerPoint conversion pipeline: each slide
// is first serialised to the canvas HTML format
// (canvas-interop.js), then addCanvasPageToPptx
// rebuilds it as native
...
// Adds ONE canvas-format page as a slide to
// an existing presentation (a Studio
// deck is many slides in one file), with native chart support.
export function addCanvasPageToPptx(pptx, htmlText) {
  ...
  doc.querySelectorAll(".canvas > .shp").forEach((shp) => {
    ...
    if (img) {
      ...
      // Generated pages inline images as data URIs; Studio images may
    ...
    for (const p of paras) {
      ...
      parts.forEach((el, j) => {
        ...
        // "Studio Display" is the heading marker, not an installed font, and
        // system-ui is CSS-only — let PowerPoint fall back to its theme font
        // for both; any other explicit family is passed through.
        let runFace = null;
        if (famRaw && !/^studio display$/i.test(famRaw) && !/^system-ui$/i.test(famRaw)) runFace = famRaw;
...
function addNativeChart(pptx, slide, c, pos, bgHex) {
  ...
  const hex = (col, fallback = "6C5CE7") => (cssColor(col) || { color: fallback }).color;
  const seriesData = series.map((s) => ({ name: s.label, labels, values: s.values || [] }));
  const seriesColors = series.map((s) => hex(s.color));
  const legendColor = hex(c.legend, "E9ECFF");
  const axisColor = hex(c.axis, "E9ECFF");
  const common = {
    ...pos,
    ...
    catGridLine: { style: "none" }, valGridLine: { color: hex(c.grid, "161D38"), style: "dash" },
    plotArea: { fill: { color: "0B1026", transparency: 100 } },
  };
  ...
  switch (kind) {
    ...
    case "waterfall": {
      ...
      slide.addChart(pptx.charts.BAR, [
        ...
        chartColors: [bgHex || "0B1026", hex(series[0]?.color, "00D4FF"), "FF8A65"],
      });
      return;
    }
    default:
